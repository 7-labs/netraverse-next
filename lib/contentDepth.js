...
const STATIC_PAGE_DEPTH = {
  ...
  'game-checker': [
    {
      heading: 'Why a single-game lookup still needs context',
      paragraphs: [
        'A single game result is helpful, but it should not be treated as the whole Linux gaming decision. The game may depend on Proton, a launcher, GPU drivers, anti-cheat, controller input, cloud saves, mods, DLC, and online services. The page gives a quick answer first, then pushes the user toward the full game page and gaming guide for the deeper migration call.',
        'Use this tool when one title is the question in your head. If that title is a casual single-player game with strong Proton support, Linux may be easy. If it is a daily multiplayer game with blocked anti-cheat, the honest answer may be Linux for everything else plus Windows, console, or cloud gaming for that one title.',
      ],
    },
    {
      heading: 'How to test the result safely',
      paragraphs: [
        'Testing should include more than launching the menu. Start the game, load a save, test graphics settings, controller input, voice chat, cloud saves, DLC, mods, and online play if the title uses it. For multiplayer games, read the anti-cheat label before logging into a main account. Avoid unsupported setups that could trigger account problems.',
        'If the game works, keep a note of the Proton version, launch options, driver, and distro used. If it does not work, do not treat that as a failure of the entire migration. It may simply define the fallback: keep Windows for that game while moving work, browsing, communication, and other games to Linux.',
        'For a must-play title, repeat the test after a major game update or driver update before removing Windows. Game compatibility is a moving target, not a one-time checkbox.',
      ],
    },
  ],
};
