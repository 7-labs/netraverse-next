...
// An overlay record carrying `redirectTo` is a tombstone for a near-duplicate slug
// (alias-dedup): it suppresses the page here and a Cloudflare 301 in public/_redirects
// sends the old URL to the canonical slug. Survives re-ingest because the tombstone
// lives in overlay (overlay-apps.partial.json), not in the generated dataset.
const appMap = Object.freeze(
  Object.fromEntries(
    Object.entries(mergeRecords(appsGenerated, overlay.apps || {}))
      .filter(([, record]) => !record.redirectTo)
      .map(([slug, record]) => [slug, normalizeApp(record)]),
  ),
);

const gameMap = Object.freeze(
  Object.fromEntries(
    Object.entries(mergeRecords(gamesGenerated, overlay.games || {}))
      .filter(([, record]) => !record.redirectTo)
      .map(([slug, record]) => [slug, normalizeGame(record)]),
  ),
);
