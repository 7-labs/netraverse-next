...
// Plain-English definition of the readiness score, reused as a static SSR
// answer block so crawlers and answer engines see the model without running JS.
export const READINESS_SCORING = {
  summary:
...
// Pre-computed example verdicts for the static answer table on the home and tool
// pages. Picks recognisable head titles from the dataset (falling back to the
// most severe blockers), reusing analyzeItem + meta so the labels match the live
// engine. Pure build-time function: no client state.
export function buildExampleVerdicts(options = [], limit = 5) {
  const PREFERRED = [
    'microsoft-office',
    'adobe-photoshop',
    'apex-legends',
  ...
  const seen = new Set();
  const take = item => {
    if (!item || seen.has(item.slug) || picked.length >= limit) return;
    seen.add(item.slug);
    picked.push(item);
  };

  PREFERRED.forEach(slug => take(bySlug.get(slug)));

  // Backfill with whatever the dataset slice has, severe blockers first, so a
  // filtered tool (app-only or game-only) still gets a full, relevant table.
  if (picked.length < limit) {
    [...options]
      .map(item => ({ item, ...analyzeItem(item) }))
      .sort((a, b) => b.severity - a.severity || b.score - a.score)
      .forEach(({ item }) => take(item));
  }
  ...
  return picked.slice(0, limit).map(item => {
    ...
    let verdictLabel;
    if (item.kind === 'game') {
      const desktop = getDesktopLinuxMeta(item.desktopLinuxStatus);
      const antiCheat = getAntiCheatMeta(item.antiCheatStatus);
      verdictLabel =
        analysis.severity === 2
          ? `Keep Windows (${antiCheat.label.toLowerCase()})`
          : desktop.label;
    } else {
      const verdict = getAppVerdictMeta(item.verdict);
      verdictLabel = verdict.label;
    }
    return {
      title: item.title,
      kind: item.kind,
      href: item.href || (item.kind === 'game' ? `/games/${item.slug}` : `/apps/${item.slug}`),
      verdictLabel,
      reason: analysis.reason,
      severity: analysis.severity === 2 ? 'high' : analysis.severity === 1 ? 'medium' : 'low',
    };
  });
}
